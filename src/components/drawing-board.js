...
import DrawingPad from "../helpers/drawing-pad";
...
export default class DrawingBoard extends Component {
  ...
  render() {
    ...
    return (
      <div>
        <h1 className="mainColor">White Board</h1>
        <div className="wrapper">
          <div className="leftPane">
            <img
              src={pen}
              className={`padding10 large cursorP margin10 ${this.getCss(1)}`}
              alt="img"
              onClick={() => this.startDrawing(1)}
            />
            <div>
              <img
                src={dot}
                className={`small margin10 cursorP ${this.getCss("small")}`}
                alt="img"
                onClick={() => this.updatePx("small")}
              />
              <img
                src={dot}
                className={`medium margin10 marginB6 cursorP ${this.getCss(
                  "medium"
                )}`}
                alt="img"
                onClick={() => this.updatePx("medium")}
              />
              <img
                src={dot}
                className={`large margin10 marginB4 cursorP ${this.getCss(
                  "large"
                )}`}
                alt="img"
                onClick={() => this.updatePx("large")}
              />
            </div>
            <div>
              <span>Pen Color : </span>
              <input
                ...
                id="favcolor"
                name="favcolor"
                className={`margin10 cursorP`}
                value={penColor}
                onChange={(e) => this.onChangeColor(e, "penColor")}
              ></input>
            </div>
            <img
              src={eraser}
              id="erase"
              className={`padding10 margin10 large cursorP ${this.getCss(2)}`}
              alt="img"
              onClick={() => this.startErase(2)}
            />
            <div>
              <img
                src={highlighter}
                id="highlight"
                className={`padding10 margin10 large cursorP ${this.getCss(3)}`}
                alt="img"
                onClick={() => this.startHighlighting(3)}
              />
            </div>
            <div>
              <span>HL Color : </span>
              <input
                ...
                onChange={(e) => this.onChangeColor(e, "highLighter")}
              ></input>
            </div>
          </div>
